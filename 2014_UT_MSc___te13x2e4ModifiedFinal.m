...
OMG1IN=0.0000001; % Water
OMG2IN=0.850;     % Acetone
OMG3IN=1-OMG2IN-OMG1IN; % CA
% Maximum Value for U or H
UTOP1=0.1600;
UTOP2=0.1229;
...
FUJB=3.37608e-3;
% Creating Mesh ...
L0=75e-4; % Fixed Thickness
Number=1e4; % Number of Nodes - Only Upper 1% of Nodes are Real
dz=L0/Number;
X=0:dz:L0;
% Time
timeT=1e-9;
...
for iIter=1:1:lt
    ...
    for jIter=2:1:lx-1
        ...
        rho=RHO;
        % Calculating ....
        ABODON=GAMMA+ALPHA*ijOMG1(jIter,iIter)+BETA*ijOMG2(jIter,iIter); 
        % Dominator of PHI
        ...
        if PHI1<0 || PHI2<0 || PHI3<0
            ...
            % Calculuating the self diffusion coefficients 
            ...
            for ii=1:2
                ...
                Q2=-V12+V13+(PHI2+PHI3)*(G12-G13)+V12*(PHI2-PHI3)*G23+...
                    U1*U2*(U2-U1-PHI1)*DG12+V12*PHI3*(3*PHI2-PHI3)*DG23-...
                    ((U1*U2)^2)*DDG12+V12*PHI2*(PHI3^2)*DDG23;
                S1=-V21+V23+(PHI1+PHI3)*(V21*G12-G23)+V21*(PHI1-PHI3)...
                    *G13+V21*U1*U2*(PHI2+U2-U1)*DG12+PHI3*...
                    (3*PHI2-1)*DG23-V21*((U1*U2)^2)*DDG12+PHI2...
                    *(PHI3^2)*DDG23;
                S2=(1/PHI2)-1+V23+V21*PHI1*(G13-G12)-(PHI1+2*PHI3)*G13+...
                    V21*(U1^2)*(2*U1-PHI2)*DG12+PHI3*(4*PHI2+PHI1-2)...
                    *DG23+V21*(U1^3)*U2*DDG12+PHI2*(PHI3^2)*DDG23;
                % Calculating ....
                ABODON=GAMMA+ALPHA*ijOMG1(jIter,iIter)+...
                    BETA*ijOMG2(jIter,iIter); 
                DP1O1=ETA*(BETA*ijOMG2(jIter,iIter)+GAMMA)/(ABODON^2);
                DP2O1=-LAMBDA*ijOMG2(jIter,iIter)*ALPHA/(ABODON^2);
                DP1O2=-ETA*ijOMG1(jIter,iIter)*BETA/(ABODON^2);
                DP2O2=LAMBDA*(ALPHA*ijOMG1(jIter,iIter)+GAMMA)/(ABODON^2);
            ...
            % Calculating ....
            dDM1O1dz=(Temp1(2)-Temp1(1))/dz/2;
            dDM1O2dz=(Temp2(2)-Temp2(1))/dz/2;
            dDM2O1dz=(Temp3(2)-Temp3(1))/dz/2;
            dDM2O2dz=(Temp4(2)-Temp4(1))/dz/2;
            % Calculating ...
            DUMMY=(ijOMG1(jIter,iIter)*ijOMG2(jIter,iIter)*...
                WM1*WM2*(WM3^3))^2;
            TERM1=ijOMG2(jIter,iIter)*WM1*WM2*(WM3^2);
            TERM2=WM1*WM3*XI12*XI23*ijOMG2(jIter,iIter);
            TERM3=WM2*WM3*XI13*XI12*ijOMG1(jIter,iIter);
            TERM4=WM1*WM2*XI13*XI23*ijOMG3(jIter,iIter);
            TERM5=ijOMG1(jIter,iIter)*ijOMG2(jIter,iIter)...
                *WM1*WM2*(WM3^2);
            ...
            SENT2=(-TERM8*(TERM9+TERM10+TERM11)+...
                TERM12*(TERM13-TERM14))/DUMMY;
            diffBDCA=SENT1*dOMG1dz(jIter,iIter)+SENT2*dOMG2dz(jIter,iIter);
            dA=(-(XI12*ijOMG2(jIter,iIter)/WM2+...
                XI13*(1-ijOMG1(jIter,iIter))...
                /WM3)/(ijOMG1(jIter,iIter)))*dOMG1dz(jIter,iIter)+...
                dOMG2dz(jIter,iIter)*(XI12/WM2-XI13/WM3)...
                /ijOMG1(jIter,iIter);
            dB=0;
            ...
            % New 
            ijOMG1(jIter,iIter+1)=ijOMG1(jIter,iIter)...
                -(dOMG1dz(jIter,iIter)*vel+d2OMG1dz(jIter,iIter)...
                *dJ1/rho)*dt;
            ijOMG2(jIter,iIter+1)=ijOMG2(jIter,iIter)...
                -(dOMG2dz(jIter,iIter)*vel+d2OMG2dz(jIter,iIter)...
                *dJ2/rho)*dt;
            ijOMG3(jIter,iIter+1)=1-...
                (ijOMG1(jIter,iIter+1)+ijOMG2(jIter,iIter+1));
        end
